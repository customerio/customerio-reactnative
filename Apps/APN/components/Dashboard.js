...
const Dashboard = ({ navigation }) => {
  ...
  const renderDashboardButtons = (item) => {
    return <ThemedButton onPress={item.onClick} title={item.key} />;
  };
    ...
    <View style={styles.container}>
      <View style={{ flex: 1, backgroundColor: 'white', paddingTop: 50 }}>
        <View style={{ justifyContent: 'space-around', flexDirection: 'row' }}>
          <View
            style={{ height: 50, alignSelf: 'stretch', margin: 5, flex: 1 }}
          >
            <View style={styles.userIconView}>
              <TouchableOpacity onPress={() => userIconTapped()}>
                <Image
          ...
          <View style={styles.featuresView}>
            <FlatList
              data={[
                ...
                {
                  key: 'Get current push permissions',
                  onClick: () => getCurrentPushPermission(),
                },
                {
                ...
                },
                {
                  ...
                  onClick: () => sendCustomEventTapped(),
                },
                {
                  key: 'Set Device Attributes',
                  onClick: () => setDeviceAttributesTapped(),
                },
                {
                  key: 'Set Profile Attributes',
                  onClick: () => setProfileAttributesTapped(),
                },
