...
class NativeMessagingPushModule(
     ...
     * has been completed.
     *
     * @param options configurations options for push notifications, required for iOS only,
     * unused on Android.
    ...
    private fun checkPushPermissionStatus(reactContext: ReactContext): PermissionStatus {
        ...
        return if (Build.VERSION.SDK_INT < BUILD_VERSION_CODE_TIRAMISU || ContextCompat.checkSelfPermission(
    ...
    companion object {
        ...
        internal fun addNativeModuleFromConfig(
            ...
            val pushClickBehavior =
                androidConfig.getTypedValue<String>(Keys.Config.PUSH_CLICK_BEHAVIOR)
                    ?.takeIfNotBlank()
                    ?.let { value ->
