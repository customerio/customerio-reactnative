...
class CioLoggerWrapper: CioLogger {
    ...
    private var nativeLogger: CioLogger
    ...
    static func getInstance (moduleRegistry: RCTModuleRegistry, logLevel: CioLogLevel) -> CioLogger {
        ...
        let wrapper = CioLoggerWrapper(moduleRegistry: moduleRegistry, nativeLogger: DIGraphShared.shared.logger, logLevel: logLevel)
        DIGraphShared.shared.override(value: wrapper, forType: CioLogger.self)
        return wrapper
    }
    
    private init(moduleRegistry: RCTModuleRegistry, nativeLogger: CioLogger, logLevel: CioLogLevel?) {
